...
function HelpDialog({ onClose, onNavigate }: { onClose: () => void; onNavigate: (section: Section) => void }) {
  const actions: Array<{ title: string; detail: string; section: Section; icon: IconName }> = [
    { title: 'Revisar viajes pendientes', detail: 'Confirma solicitudes sin conductor y abre el detalle operativo.', section: 'trips', icon: 'trips' },
    { title: 'Asignar un conductor', detail: 'Usa la cola de asignación para elegir un conductor disponible.', section: 'assignment', icon: 'assignment' },
    { title: 'Monitorear operaciones', detail: 'Abre el tracking para consultar posiciones GPS reales.', section: 'tracking', icon: 'tracking' },
    { title: 'Atender incidencias', detail: 'Consulta evidencias, cambia el estado y deja trazabilidad.', section: 'incidents', icon: 'incidents' },
  ]
...
function DriverEditDialog({ driver, vehicles, onClose, onSaved, onVehicleChanged, onError }: { driver: Driver; vehicles: Vehicle[]; onClose: () => void; onSaved: (driver: Driver) => void; onVehicleChanged: (vehicle: Vehicle) => void; onError: (message: string) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      onError(error instanceof Error ? error.message : 'No se pudo actualizar el conductor')
    } finally {
      setBusy(false)
    }
  }
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <form className="modal-card wide driver-edit-modal" onSubmit={submit}>
      <div className="modal-header"><div><span className="eyebrow">OPERACIONES · CONDUCTORES</span><h2>Editar conductor</h2><p>Actualiza el expediente y asigna una unidad cargada en Vehículos y flota.</p></div><button type="button" className="icon-button" onClick={onClose} aria-label="Cerrar">×</button></div>
      <div className="assignment-callout"><span className="assignment-callout-icon"><Icon name="vehicles" size={15} /></span><div><strong>Asignación de vehículo</strong><small>El vehículo queda vinculado al conductor y no se puede seleccionar si ya está en uso.</small></div></div>
      <div className="form-grid">
        <label>Nombre completo<input required value={name} onChange={(event) => setName(event.target.value)} /></label>
...
function onNoticeSafe(onError: (message: string) => void, message: string) { onError(message) }
...
function DriversView({ drivers, vehicles, onNavigate, onNotice, onDeleted, onDriverChanged, onVehicleChanged }: { drivers: Driver[]; vehicles: Vehicle[]; onNavigate: (section: Section) => void; onNotice: (message: string) => void; onDeleted: (id: string) => void; onDriverChanged: (driver: Driver) => void; onVehicleChanged: (vehicle: Vehicle) => void }) {
